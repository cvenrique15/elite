...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["actualizar"])) {
    ...
    $consulta_existencia = "SELECT id_proveedor FROM proveedores WHERE (telefono = :telefono AND telefono <> :telefono_anterior) OR (correo = :correo AND correo <> :correo_anterior)";
    ...
    if ($resultado_existencia) {
        ...
        $consulta_actualizar = "UPDATE proveedores SET nombre = :nombre, apellido = :apellido, raza = :raza";
        $params = array(':nombre' => $nombre, ':apellido' => $apellido, ':raza' => $raza);
        ...
        $consulta_actualizar .= " WHERE id_proveedor = :id_proveedor";
        $stmt_actualizar = $conexion->prepare($consulta_actualizar);
        foreach ($params as $key => &$val) {
            $stmt_actualizar->bindParam($key, $val);
        }
        $stmt_actualizar->bindParam(':id_proveedor', $_SESSION["id_proveedor"]);
...
<head>
    ...
    <title>Gestión de Cuenta - Proveedor</title>
...
<body>
    <?php if (isset($_SESSION["id_proveedor"])) : ?>
        ...
        <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="POST">
            ...
            <input type="text" id="raza" name="raza" value="<?php echo $_SESSION["raza"]; ?>" <?php if ($_SESSION["raza"] == $raza) echo 'class="no-actualizado"'; ?>><br>
